...
| 分类 | 带颜色的标签 / 分类 |
| 归处（首页） | **全局搜索框**（搜名称/位置/分类/标签/序列号）+ **临期清理工作台**（已过期/临期分段、勾选批量归档/丢弃）+ 常用位置快捷入口 + 最近搜索（统计移至底部边缘） |
| 筛选 | 物品按关键词 / 状态 / 分类 / 位置四维组合搜索 |
| 图片 | 拍照上传 → 自动 WebP 压缩 → 超 2000px 缩放 → 缩略图预览 |
| 通知 | Web Push 推送：物品过期前 3 天自动提醒（含保修期，每 6 小时扫描） |
| URL 路由 | 每个视图/展开/搜索状态有独立 URL（`#/items?kw=冰箱`），浏览器前进/后退逐级回溯、刷新保留 |
...
> 当前版本：**v0.9.6** — 新首页「归处」（全局搜索 + 临期清理工作台）+ hash 路由（每页独立 URL、逐级后退）+ 位置拖拽边缘自动滚动 + 多展开状态恢复。详见 [更新日志](docs/02-更新日志(CHANGELOG).md)。
