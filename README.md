...
| 物品 | 名称、描述、数量、单位、**状态**、**保质期**（天数自动算到期）、生产日期、**图片附件**、条形码/序列号/保修期 |
| 位置 | **方块卡片层级树**（显示物品数量）+ 编辑模式拖拽调层级（**边缘自动滚动**）+ 点击展开物品列表（**多展开状态可逐级后退**）+ 内联添加/改名 + 编辑模式自动收起物品 |
| 分类 | 带颜色的标签 / 分类 |
| 归处（首页） | **全局搜索框**（搜名称/位置/分类/标签/序列号）+ **临期清理工作台**（已过期/临期分段、「已处理」一键清理）+ 常用位置快捷入口 + 最近搜索（统计移至底部边缘） |
| 物品页 | **主-从布局**：左侧紧凑列表（名称/📍位置/数量/保质期角标）+ 右侧详情卡片（点击列表行即更新，操作按钮集中卡片） |
| 添加物品 | **独立页面** `#/add`（顶栏 ＋ 进入）：扫码自动回填曾录入信息、分组表单（条码/基本信息/照片/保质期/数量与状态/更多信息）、名称/位置/照片必填、标签按钮式多选 + 直接新建 |
| 筛选 | 关键词 / 状态 / 分类 / 位置 + **标签多选**（chips 并集） + **4 种排序**（最新添加/保质期/位置/分类） |
| 图片 | 拍照上传 → 自动 WebP 压缩 → 超 2000px 缩放 → 缩略图预览 |
| 通知 | Web Push 推送：物品过期前 3 天自动提醒（含保修期，每 6 小时扫描） |
| URL 路由 | 每个视图/展开/搜索/选中状态有独立 URL（`#/items?kw=冰箱&sort=expiry&tag_ids=1&sel=3`、`#/add?id=5`），浏览器前进/后退逐级回溯、刷新保留 |
...
> 当前版本：**v0.9.13** — 物品页主-从布局（紧凑列表 + 详情卡片）+ 独立添加页（扫码记忆、必填校验）+ 排序/标签多选 + 表单分组重排与布局收紧。详见 [更新日志](docs/02-更新日志(CHANGELOG).md)。
