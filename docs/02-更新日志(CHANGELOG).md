...
#### 修复
- **「数量与状态」分组区域过高（215→93px）**：根因是 `.card` 基类自带 `flex-wrap: wrap`，与 `.add-form` 的 `flex-direction: column` 组合，在 Chromium 中将每个 fieldset 子项高度拉伸到 ~220px（实测内容只有 93px）。修复：`.add-form` 显式设 `flex-wrap: nowrap`
- **「更多信息」分组（258→197px）**：同根因，附带受益
- **页面总高 1597→1351px（-246px）**：整体明显更紧凑

#### 验证
- 桌面 1280px + 手机 430px 截图确认各分组高度正常
- pytest 36 passed
